...
// 方法二：二分查找 + 前缀数组
// 本题标签有二分查找，那就讲一下二分查找大致思路。
//  1. 首先和方法一一样，先对 worker 和 difficulty 进行相同的预处理，即对工人按照能力升序排序，对工作难度和报酬的合并后按照难度升序排序。
//  2. 此时对于每个工人，对其能力 w，可以快速通过「二分查找」快速找到难度不大于 w 的最高收益工作 jobs[i]。
//  3. 我们要找的是 [0, i] 范围内工作收益的最大值，因此需要对工作数组预先计算前缀树组，保存每个位置对应的收益最大值；
